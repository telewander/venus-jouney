...
const files=[];async function walk(dir){for(const e of await fs.readdir(dir,{withFileTypes:true})){const p=path.join(dir,e.name);if(e.isDirectory())await walk(p);else files.push(p.replaceAll('\\','/'))}}await walk('dist');
...
assert(!sitemap.includes('/404'),'sitemap must not include the 404 page');
const robots=await fs.readFile('dist/robots.txt','utf8');
assert.match(robots,/Sitemap:\s+\S*sitemap\.xml/);
console.log(`PASS: ${htmlFiles.length} pre-rendered pages, 9 sitemap URLs, unique titles, H1s, structured data, local links, ${images} image references and contact destinations.`);
